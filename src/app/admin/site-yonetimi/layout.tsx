...
export default function SiteYonetimiLayout({ children }: { children: React.ReactNode }) {
  const menuItems = [
    { group: "İçerik", items: [
      { href: "/admin/site-yonetimi/sayfalar", label: "📄 Sayfalar", icon: <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg> },
      { href: "/admin/site-yonetimi/yazilar", label: "📰 Blog Yazıları", icon: <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M19 20H5a2 2 0 01-2-2V6a2 2 0 012-2h10a2 2 0 012 2v1m2 13a2 2 0 01-2-2V7m2 13a2 2 0 002-2V9a2 2 0 00-2-2h-2m-4-3H9M7 16h6M7 8h6v4H7V8z" /></svg> },
      { href: "/admin/site-yonetimi/medya", label: "🖼️ Medya", icon: <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg> },
    ]},
    { group: "Sistem", items: [
  ...
  return (
    <div>
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-warm-900">🔧 Site Yönetimi</h1>
        <p className="text-warm-500 text-sm mt-1">İçerik, sistem, güvenlik ve bakım yönetimi</p>
      </div>
      <nav className="flex flex-wrap gap-1 mb-8 bg-white rounded-xl border border-warm-200 p-2 overflow-x-auto">
        {menuItems.flatMap(g => g.items).map(item => (
          <Link key={item.href} href={item.href} className="flex items-center gap-1.5 px-3 py-2 rounded-lg text-xs font-medium text-warm-600 hover:bg-warm-50 hover:text-primary transition-colors whitespace-nowrap">
            {item.icon}
            {item.label}
          </Link>
        ))}
